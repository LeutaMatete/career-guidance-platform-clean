...
const InstituteRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('✅ Institute registration successful! Check your REAL email for verification link from Firebase.');
      navigate('/auth/institute/verify-email');
    } catch (err) {
  ...
  const formatPhoneDisplay = (phone) => {
    ...
    if (/^\d+$/.test(cleanPhone) && cleanPhone.length <= 8) {
      ...
      return `${cleanPhone.slice(0,2)} ${cleanPhone.slice(2,4)} ${cleanPhone.slice(4,6)} ${cleanPhone.slice(6,8)}`;
    }
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        <h2>Create Institute Account</h2>
        <p className="auth-subtitle">Join CareerGuide as an Educational Institution - Real email verification required</p>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <div className="form-notice">
            <p>📧 <strong>Real Email Required:</strong> You will receive a verification link from Firebase to activate your institute account.</p>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? 'Creating Institute Account...' : 'Create Institute Account'}
          </button>
        </form>

        <div className="auth-footer">
          <p>
            Already have an institute account? <Link to="/auth/institute/login">Sign In as Institute</Link>
          </p>
        </div>
      </div>
    </div>
